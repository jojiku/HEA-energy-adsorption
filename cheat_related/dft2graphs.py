...
e_h20 = ase.io.read('examples/gpaw/refs/h2o.traj',-1).get_potential_energy()
e_h2 = ase.io.read('examples/gpaw/refs/h2.traj',-1).get_potential_energy()
...
for s in ['train','val','test']:     
    slab_paths = glob.glob(f'examples/gpaw/{s}/*_slab.traj')
    graph_list = []
    ...
    for sp in tqdm(slab_paths,total=len(slab_paths)):
        ...
        ads_paths = glob.glob(f'examples/gpaw/{s}/{str(slabId).zfill(4)}_ads*.traj')
    ...
    with open(f'examples/train_lgnn/graphs/{s}.graphs', 'wb') as output:

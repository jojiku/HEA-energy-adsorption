...
def identify_surface_layers(atoms, z_tolerance=0.5):
    ...
    # FIX: Assign surface layer (tag=1) to the TOPMOST layer
    # Reverse the layer assignment so highest z gets tag=1
    layer_dict = {z: len(sorted_z)-i for i, z in enumerate(sorted_z)}
...
def process_data_directories_site_specific(root_dir="C:/Users/Tseh/Documents/Files/HEA/hea_project"):
    source_dirs = {
        "pairs": os.path.join(root_dir, "train_data_dft/HEA_results_pairs"),
        "triplets": os.path.join(root_dir, "train_data_dft/HEA_results_triplets"),
        "fives": os.path.join(root_dir, "train_data_dft/HEA_results_fives")
    }
    
    output_dirs = {
        "pairs": os.path.join(root_dir, "train_data_graphs/graphs_site_specific", "pairs"),
        "triplets": os.path.join(root_dir, "train_data_graphs/graphs_site_specific", "triplets"),
        "fives": os.path.join(root_dir, "train_data_graphs/graphs_site_specific", "fives")
    }
